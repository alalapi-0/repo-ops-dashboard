#!/usr/bin/env python3
"""Generate a Cursor prompt from a governance task_spec YAML file."""
...
def validate_task_spec(spec: dict[str, Any], source: str = "") -> list[str]:
    ...
    if agent and agent != "Cursor":
        errors.append(f"{prefix}assigned_agent must be Cursor for this generator (got {agent!r})")
    return errors
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Cursor prompt from task_spec YAML")
    parser.add_argument(
        "--task-spec",
        default="governance/task_specs/example_task_spec.yaml",
        help="Input task_spec YAML path",
    )
    parser.add_argument(
        "--template",
        default="prompts/cursor_from_task_spec.md",
        help="Prompt template path",
    )
    parser.add_argument(
        "--output",
        default="",
        help="Output markdown path (default: prompts/generated/<task_id>_cursor.md)",
    )
...
def main() -> int:
    ...
    out_path = Path(args.output) if args.output else Path("prompts/generated") / f"{safe_id}_cursor.md"

    print(f"[cursor_prompt] task={task_id} chars={len(content)} dry_run={args.dry_run}")
    print(f"[cursor_prompt] output={out_path}")
    if not args.dry_run:
