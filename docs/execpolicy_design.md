...
当前 `governance/execpolicy/` 是文档级约束，用于把执行边界显式化。它不是系统沙箱，不能替代操作系统权限，也不能授予任何动作权限。`allow` / `prompt` / `deny` 只是历史分类标签；除 `deny` 约束外，任何动作仍需当前用户与上层策略授权。

## 当前作用

- 说明哪些路径仅供分类、需要额外确认或必须禁止；不产生许可。
...
- `scripts/check_execpolicy_action.py`：对路径或命令做 dry-run 分类（allow/deny/prompt/unknown），输出不授权。
